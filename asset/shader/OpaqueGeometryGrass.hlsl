...
//sample color
float3 sampleColorSmooth(float2 uv, float3 color1, float3 color2, float3 color3)
{
    float y = uv.y;
    
    float weight1 = 1.0 - smoothstep(0.0, 0.5, y);
    float weight2 = smoothstep(0.0, 0.5, y) - smoothstep(0.5, 1.0, y);
    float weight3 = smoothstep(0.5, 1.0, y);
...
)
{
    ...
    float3 Albedo = sampleColorSmooth(i_uv, topColor, middleColor, bottomColor) * albedo.xyz;
