...
namespace GuGu {
	...
	static Array<uint8_t> converColorToBytes(Color inColor)
	{
		Array<uint8_t> res;
		res.resize(4 * sizeof(float));
		inColor.r = HostToLittleFloat(inColor.r);
		inColor.g = HostToLittleFloat(inColor.g);
		inColor.b = HostToLittleFloat(inColor.b);
		inColor.a = HostToLittleFloat(inColor.a);
		std::memcpy(res.data(), &inColor, 4 * sizeof(float));
		return res;
	}

	static TrackData createLightTrackData()
	{
		TrackData trackData;
		trackData.m_isEvent = false;
		meta::Type currentType = typeof(Color);
		...
		section.m_bindingObjectName = u8"SunLight";
		//component type guid
		meta::Type lightType = typeof(GuGu::LightComponent);
		section.m_bindingObjectComponentTypeGuid = lightType.getGuid().getGuid();

		Array<CutscenesKeyFrame> keyFrames;

		CutscenesKeyFrame keyFrame_1;
		CutscenesKeyFrame keyFrame_2;
		keyFrame_1.m_fieldName = "m_lightColor";
		keyFrame_1.m_timestamp = 2.0f;
		keyFrame_1.m_typeGuid = currentType.getGuid().getGuid();
		Color keyFrame_1Value = Color(1.0f, 0.1f, 0.7f, 1.0f);
		keyFrame_1.m_value = converColorToBytes(keyFrame_1Value);

		keyFrame_2.m_fieldName = "m_lightColor";
		keyFrame_2.m_timestamp = 17.0f;
		keyFrame_2.m_typeGuid = currentType.getGuid().getGuid();
		Color keyFrame_2Value = Color(0.0f, 0.4f, 0.6f, 1.0f);
		keyFrame_2.m_value = converColorToBytes(keyFrame_2Value);
	...
	void testCutscenes()
	{
		...
		//create light component section
		TrackData lightTrackData = createLightTrackData();

		trackData.m_sections = sections;
		trackDatas.push_back(trackData);
		trackDatas.push_back(lightTrackData);
