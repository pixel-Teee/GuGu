...
namespace GuGu {
	...
	bool ViewportClient::onMouseButtonUp(UIPointerData uiPointerData)
	{
		...
		if (getViewportState() == ViewportState::Runtime)
		{
			std::shared_ptr<InputManager> inputManager = InputManager::getInputManager();
			std::shared_ptr<UIComponent> capturedUIComponent = inputManager->getCaptureUIComponent();
			if(capturedUIComponent)
			{
				//capture
			...
			else
			{
				...
				if (collidedUIGameObject)
				{
					...
					while (currentGameObject)
					{
						...
						if (uiComponent)
						{
							uiComponent->onPointerMove(uiPointerData);
						}
						currentGameObject = currentGameObject->getParentGameObject().lock();
					}
					return true;
