...
namespace GuGu {
	namespace meta {
		...
		Global::Global(const GuGuUtf8Str& name, Type type, GlobalGetterBase* getter, GlobalSetterBase* setter, Type parentType /*= Type::Invalid() */)
		: m_type(type)
		...
		const GuGuUtf8Str& Global::GetName(void) const
		{
			return m_name;
		}
