...
namespace GuGu {
	Border::Border()
		...
		, m_imageBursh(CoreStyle::getStyleSet()->getBrush("NoBorder"))
	{
	}
	Border::~Border()
	{
	...
	uint32_t Border::onGenerateElement(PaintArgs& paintArgs, const math::box2& cullingRect, ElementList& elementList, const WidgetGeometry& allocatedGeometry, uint32_t layer)
	{
		...
		{
			ElementList::addBoxElement(elementList, allocatedGeometry, m_imageBursh.Get()->getTintColor() * m_borderBackgroundColor.Get(), m_imageBursh.Get(), layer); //background
		}
