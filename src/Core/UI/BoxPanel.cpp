	...
	static void ArrangeChildrenAlong(const std::vector<SlotType>& childrens, const WidgetGeometry& widgetGeometry, ArrangedWidgetArray& arrangedWidgetArray)
	{
		if (childrens.size() > 0)
		{
			...
			for (size_t i = 0; i < childrens.size(); ++i)
			{
				...
				arrangedWidgetArray.pushWidget(childVisibility, widgetGeometry.getChildGeometry(curChild->getChildWidget(), math::float2(localSize.x, localSize.y), math::float2(localPosition.x, localPosition.y)), curChild->getChildWidget());
